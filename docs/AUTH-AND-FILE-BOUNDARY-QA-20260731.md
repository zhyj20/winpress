...
- 后端 Maven 测试通过。
- 本段为 2026-07-31 的本机验收快照：当时健康检查返回数据库 `UP`、结构版本 `36`、接口契约 `winpress-v4.2.25-20260731`。当前匿名健康响应已按后文“公共健康信息边界”收口，不再公开版本或接口合同。
...
- 前端 `npm.cmd run check` 与 `npm.cmd run build` 通过；当时的 Playwright 专项回归为桌面端和移动端 `44/44` 通过。后续完整三视口回归结果见本文件 2026-08-01 补充记录。
...
- 经 2026-08-01 复核，早期记录将一次自动化运行器被中断后的清理结果误写为“通过”。该次生产等价冷启动没有可追溯的完整退出码与最终通过输出，因此不计入验收；临时容器、卷、网络、镜像、凭据和状态文件的清理路径已验证。完整冷启动仍须在可持续运行的预发布执行器中重跑并留存非敏感日志。
...
- `verify-local-p0p1.ps1`、隔离数据库恢复、隔离上传卷恢复、干净本机数据库初始化和生产 Compose 静态边界检查均通过。
- **2026-08-01 完整浏览器回归：** 将 Playwright 按项目串行拆分为 `desktop`、`tablet`、`mobile` 三轮运行，每轮 `36/36`；三轮均生成 `.last-run.json`，状态为 `passed`、`failedTests: []`，合计 `108/108`。供应商订单状态选择器修补后又完整重跑一次，结果仍为 `108/108`。覆盖公开营销路由、三类角色菜单与看板、媒体邀请和发布会、供应商履约管理、订单/任务记录、数据字段边界及浏览器历史筛选同步。测试进程会在父终端提前返回，但均在子进程退出后读取最终状态文件，不以中间输出判断结果。
- **2026-08-01 结算边界复跑：** 在前述快照后新增结算状态直接转换和移动结算卡断言，完整三视口回归更新为每轮 `37/37`、合计 `111/111`；`.last-run.json` 最终为 `passed`、`failedTests: []`。后端完整 Maven 测试更新为 `174/174`，非法 `PAID → CONFIRMED` 运行时请求返回 `409 / INVALID_SETTLEMENT_TRANSITION`，没有改变账务记录。该补充不改变附件、会话或对客数据可见性边界。
- 本机演示环境的生产等价冷启动仍未取得完整通过输出；清理已验证，但正式预发布冷启动、TLS、域名、监控和生产恢复演练仍是上线门禁。
